...
if ! command -v silverc >/dev/null 2>&1 && [ ! -x "$HOME/.cargo/bin/silverc" ]; then
  echo ""
  echo "=== Portrait CI FAIL: silverc not found ==="
  echo "    (the differential layer needs the real silverc; put it on PATH or"
  echo "     at \$HOME/.cargo/bin/silverc — without it the gate runs half-blind)"
  exit 1
fi
