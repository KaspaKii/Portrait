...
|---|---|---|---|---|
| `KCP_NODE_URL` | every node example: `kcp-common`(`covenant_auditor`, `node_status`, `p2sh_roundtrip`, `reserve_covenant_live`), `kcp-ktt-token`(`ktt_testnet_evidence`), `kcp-paired-attestation`(`paired_attestation_onchain_evidence`, `paired_attestation_testnet_evidence`), `kcp-sealed-lineage`(`sealed_lineage_testnet_evidence`), `kcp-transferable-record`(`transferable_record_testnet_evidence`), `kcp-vault`(`vault_testnet_evidence`, `vault_onchain_evidence`, `composite_evidence`) | No | `ws://localhost:17210` | wRPC (Borsh) URL of the Kaspa testnet node to connect to. Several examples print a notice to stderr when falling back to the default. |
| `KCP_KEY_FILE` | `kcp-common`(`p2sh_roundtrip`, `reserve_covenant_live`), `kcp-ktt-token`(`ktt_testnet_evidence`), `kcp-paired-attestation`(`paired_attestation_onchain_evidence`, `paired_attestation_testnet_evidence`), `kcp-sealed-lineage`(`sealed_lineage_testnet_evidence`), `kcp-transferable-record`(`transferable_record_testnet_evidence`), `kcp-vault`(`vault_testnet_evidence`, `vault_onchain_evidence`, `composite_evidence`) | **Yes** (where used) | none | Path to the funded testnet wallet key file (mnemonic or raw-hex) that signs/funds the transactions. Hard error if unset — except in `reserve_covenant_live`, where it is not required when `KCP_DRY_RUN` is set. In the `*_onchain_evidence`/`composite_evidence`/`reserve_covenant_live`/`p2sh_roundtrip` examples it is index 0 of a 2-of-2 or the sole signer. |
| `KCP_NEXT_KEY_FILE` | `kcp-paired-attestation`(`paired_attestation_onchain_evidence`), `kcp-vault`(`vault_onchain_evidence`, `composite_evidence`), `kcp-transferable-record`(`transferable_record_testnet_evidence`) | Mixed | none | Path to a **second, distinct** key file. **Required** in paired-attestation `paired_attestation_onchain_evidence` (oracle B's key — needs no funds). **Optional** elsewhere: in vault `vault_onchain_evidence`/`composite_evidence` it supplies the second 2-of-2 signer, falling back to BIP-44 index 1 of `KCP_KEY_FILE` when unset (which only differs for a mnemonic key file); in transferable `transferable_record_testnet_evidence` it is the "next controller", likewise falling back to index 1. |
...
| `KCP_TS` | `kcp-ktt-token`(`ktt_testnet_evidence`), `kcp-paired-attestation`(`paired_attestation_testnet_evidence`), `kcp-sealed-lineage`(`sealed_lineage_testnet_evidence`), `kcp-transferable-record`(`transferable_record_testnet_evidence`), `kcp-vault`(`vault_testnet_evidence`) | No | none (omitted) | ISO-8601 timestamp for reproducibility across runs. In paired-attestation, sealed-lineage, and transferable-record it is embedded into the record/genesis body when set, and omitted otherwise. In the `kcp-ktt-token` and `kcp-vault` testnet-evidence examples it is currently read but not yet wired into the payload. |
