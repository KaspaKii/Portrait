...
The five core covenant patterns below, plus shared plumbing, share one workspace
(452 tests on a fresh clone with default features, 533 with `--all-features`,
2 ignored either way, `cargo test`, verified 2026-09-01; `cargo clippy -D warnings`
clean on both feature sets). The library has since grown additional primitive and
tooling crates beyond these five — access/security/cryptography helpers, vesting,
a yield-vault profile, governance and CLI scaffolding — for thirteen crates in
all; `--all-features` (== `--features wrpc`, where the real-engine tests live) is
now part of the gate and builds clean:
