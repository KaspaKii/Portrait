...
///
/// A version-0 input commits its covenant budget as a `u8` `SigopCount`, so
/// `255 × 100_000 + 9_999 = 25,509,999` script units
/// ([`crate::sigop::MAX_COMMITTABLE_SCRIPT_UNITS`]) is the largest budget that
/// can be expressed — there is nothing above it. The shipped reference proof
/// already costs **25,446,182 units: 0.25% under the ceiling.** A larger seal, a
/// longer control-inclusion path, or a different guest can cross that line, and
/// then the spend can never be budgeted. Because these proof fields are inside
/// the redeem script, they are inside the P2SH address, so there is no other
/// spending path and **the funds are permanently unrecoverable**.
///
/// Measure your own proof shape first — the helper needs the `wrpc` feature,
/// which pulls the engine:
///
/// ```sh
...
/// assert!(kcp_pq_anchor::sigop::fits_pq_verify_budget(units));
/// ```
///
/// # Invariants
///
