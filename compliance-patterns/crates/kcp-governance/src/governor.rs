...
impl GovernorState {
    ...
    pub fn refresh_status(&mut self, current_height: u64) {
        ...
        //
        // `Rejected` is terminal too: the voting window has closed without
        // quorum, so there is no legitimate route back. `MultiSigVote::approve`
        // is public and window-blind, and every field here is `pub`, so a
        // caller can record an approval directly on `self.vote` after the
        // deadline; without this arm the next refresh would silently promote a
        // dead proposal to `Passed`. (`lineage::verify_governor_lineage`
        // already treats `Rejected` as terminal.)
        match self.status {
            ProposalStatus::Cancelled | ProposalStatus::Executed | ProposalStatus::Rejected => {}
...
mod tests {
    ...
    /// A `Rejected` proposal must stay rejected. `vote` is a public field and
    /// `MultiSigVote::approve` does not know about the voting window, so an
    /// approval can be recorded after the deadline; the next `refresh_status`
    /// must not promote the dead proposal to `Passed`.
    #[test]
    fn rejected_is_terminal_even_if_approvals_arrive_after_the_deadline() {
        let mut gov = make_governor();
        gov.refresh_status(250); // past voting_deadline=200, no approvals
        assert_eq!(gov.status, ProposalStatus::Rejected);

        // Backdoor: record quorum directly on the public vote tracker.
        gov.vote.approve(key(1)).unwrap();
        gov.vote.approve(key(2)).unwrap();
        assert!(gov.vote.quorum_met());

        gov.refresh_status(260);
        assert_eq!(
            gov.status,
            ProposalStatus::Rejected,
            "a rejected proposal must not be revived by late approvals"
        );
        assert!(gov.schedule_action(260).is_err());
        assert!(gov.execute(400).is_err());
    }
