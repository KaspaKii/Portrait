...
//! cargo run -p kcp-transferable-record --example transferable_record_testnet_evidence \
//!            --features wrpc
//! ```
//!
//! Optional:
...
async fn main() -> Result<(), BoxError> {
    ...
    // Payload invariants only: validate_chain does NOT check controller keys.
    // Custody continuity is the UTXO chain (the transfer spends the record UTXO,
    // so consensus required the previous controller's signature).
    ...
    kcp_transferable_record::lineage::validate_chain(&events)
    ...
    println!("  source: examples/transferable_record_testnet_evidence.rs");
