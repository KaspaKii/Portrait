...
//! | `owner()` | `record.owner()` |
//! | `transferOwnership(newOwner)` | `record.transfer_ownership(by, new_owner)?` |
//! | `renounceOwnership()` | `record.renounce_ownership(by)?` |
...
impl OwnershipRecord {
    ...
    ///
    /// The caller must provide the current owner's key `by` to authenticate.
    /// Returns `Err(NotOwner)` if `by` is not the current owner.
    ///
    /// On success returns a new `OwnershipRecord`. The caller must persist this
    /// and build a transaction whose outputs use the new key.
    ///
    /// Equivalent to Solidity's `transferOwnership(newOwner)` under `onlyOwner`.
    ///
    ...
    pub fn transfer_ownership(self, by: [u8; 32], new_owner: [u8; 32]) -> Result<Self> {
        self.verify_owner(by)?;
        Ok(Self {
            owner_key: new_owner,
        })
    }
    ...
    /// pattern to recover).
    ///
    /// Equivalent to Solidity's `renounceOwnership()` under `onlyOwner`.
    pub fn renounce_ownership(self, by: [u8; 32]) -> Result<Self> {
        self.verify_owner(by)?;
        Ok(Self {
            owner_key: [0u8; 32],
        })
...
mod tests {
    ...
    #[test]
    fn transfer_ownership_by_owner_succeeds() {
        let rec = OwnershipRecord::new(key(7));
        let rec2 = rec.transfer_ownership(key(7), key(8)).unwrap();
        assert_eq!(rec2.owner(), key(8));
    }

    #[test]
    fn transfer_ownership_by_non_owner_rejected() {
        let rec = OwnershipRecord::new(key(7));
        assert!(matches!(
            rec.clone().transfer_ownership(key(99), key(8)),
            Err(Error::NotOwner)
        ));
        assert_eq!(rec.owner(), key(7));
    }

    #[test]
    fn renounce_ownership_by_owner_succeeds() {
        let rec = OwnershipRecord::new(key(9));
        let rec2 = rec.renounce_ownership(key(9)).unwrap();
        assert!(rec2.is_renounced());
        assert_eq!(rec2.owner(), [0u8; 32]);
    }

    #[test]
    fn renounce_ownership_by_non_owner_rejected() {
        let rec = OwnershipRecord::new(key(9));
        assert!(matches!(
            rec.clone().renounce_ownership(key(99)),
            Err(Error::NotOwner)
        ));
        assert_eq!(rec.owner(), key(9));
    }
}
