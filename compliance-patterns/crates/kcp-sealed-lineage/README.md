...
// ... see examples/sealed_lineage_testnet_evidence.rs for the full flow
```
...
cargo run -p kcp-sealed-lineage --example sealed_lineage_testnet_evidence --features wrpc
```
...
**Assets** — the append-only integrity and ordering of the evidence log; the
confidentiality of the off-chain record bodies (protected only by the blind);
the small lineage UTXO value.

**Attacker capabilities (assumed)** — on a UTXO chain the **spender is the
adversary**: whoever holds the publisher key chooses each successor
transaction, its payload bytes (`seq`, `event_class`, `t_bucket`,
`commitment`), its outputs, and whether to append at all. A second adversary is
the **discloser**: the party who hands a verifier a set of payloads plus
`(body, blind)` pairs of their choosing. Every payload and commitment is public
to any chain observer.

**What consensus enforces** — on the default anchoring path
(`tx::{create,append}_lineage_tx`, plain pay-to-address) exactly two things: the
publisher's Schnorr signature is required to spend the lineage UTXO, and that
UTXO can be spent only **once**, so the chain cannot fork into two accepted
successors. Consensus does not read the payload. Under the separate
covenant-id-bound artifact (`covenant/sealed-lineage.compiled.json`, live as
`[KCP-SL-003]`) the script VM does enforce the lineage *structure*: L-1
`seq + 1`, L-2 identical `lineage_id`, L-3 event-class ordering (nothing after
`CLOSE`, no second `GENESIS`), L-4 the ≤ 90-day `t_bucket` envelope, and
`checkSig(publisherPk)`.

**Two things that enumeration does *not* include.** The covenant state is
`{lineage_id, seq, event_class, t_bucket, publisherPk}` — there is **no
`commitment` field**, so the blinded seal, the thing the whole pattern exists to
anchor, is **never bound by consensus on any path**; a covenant-enforced append
may carry an arbitrary commitment. And `append` leaves `newStates[0].publisherPk`
unconstrained, so the authorised publisher can **rotate the key to anyone** at
any append and the covenant will accept it. Both are properties of the committed
artifact (changing either recompiles the covenant and changes its covenant-id),
so they are documented, not patched — see `KNOWN-ISSUES.md`.

**What this assumes / trusts off-chain** — that the publisher is honest about
what it seals: `t_bucket` is publisher-supplied, not a consensus clock, so it
may be any value the envelope admits. A commitment binds a body only if the
blind is genuinely secret, retained, and the verifier recomputes the same
canonical JSON. The record body is trusted input — a lineage proves *ordering
and commitment*, never that the content is true. A verifier must obtain the
payloads from the chain independently of whoever discloses the states;
`validate_chain` on its own proves internal consistency of what it was handed.
Taking the covenant path is a **manual** step: no library API wires a payload
into `[KCP-SL-003]`, so consensus enforcement is *available*, not *automatic*.

**Known limits and non-goals** — the in-repo engine proof
(`tests/covenant_engine.rs`) runs the committed artifact through input 0's
script on the pinned v2.0.0 **script VM only**: no transaction mass, no KIP-9
storage mass, no standardness, and the covenant places no floor on the output
value. Those two gaps compound: because nothing enforces a floor and nothing
here checks KIP-9 storage mass, a script-valid append can still be rejected by
the network for mass — callers must respect `MIN_CHANGE_SOMPI` and size the
output themselves. The **live** half
rests on perishable testnet-10 txids, and the committed↔deployed correspondence
is attested by an out-of-repo byte capture plus an execution-cost fingerprint —
evidence, not a binding. On the default path a compromised publisher key can
append arbitrary payloads: the invariant library detects it, consensus does not
prevent it. Nothing prevents **withholding** — a publisher can stop appending or
never publish an event; L-4 makes staleness visible, not impossible. Value-carry
(V-5) is a property of the single-output transaction shape, not of consensus.
Commitments are SHA-256, not Poseidon, and a low-entropy body is guessable once
its blind is disclosed. Every key here roots in secp256k1 Schnorr: not
