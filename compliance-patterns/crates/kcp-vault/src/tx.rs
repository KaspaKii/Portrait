//! On-chain anchoring of vault evidence transactions.
//!
//! [`anchor_vault_tx`] creates a Kaspa transaction that:
//! - locks `vault_value_sompi` to the wallet's pay-to-address script (key-
//!   controlled in v0 — see the crate-level v0 evidence-model note),
//! - carries the vault evidence payload in the transaction payload field.
//!
//! A change output (if above [`MIN_CHANGE_SOMPI`]) returns excess funds to the
//! same wallet address.
//!
//! ## Fee handling
//!
//! Uses [`kcp_common::tx::CARRIER_FEE_SOMPI`]. Change below
//! [`MIN_CHANGE_SOMPI`] is folded into the fee to avoid dust / KIP-0009
//! storage-mass violations.
//!
//! ## Default vault value
//!
//! [`DEFAULT_VAULT_VALUE_SOMPI`] is `10_000_000` sompi (0.1 KAS). This keeps
//! the storage-mass well within the KIP-0009 cap of 10⁶ with the 70-byte
//! payload:
//!
//! ```text
//! storage_mass ≈ C · p² / amount,  C = 10¹²
//! p (output mass) ≈ 300 for a 70-byte payload output
//! storage_mass ≈ 10¹² · 300² / 10_000_000 ≈ 9_000
//! ```
//!
//! ## v0 limitation
//!
//! The vault UTXO is locked to a plain pay-to-address script. The compiled
//! covenant script bytes are only present in the *evidence payload*; the UTXO
//! itself is not encumbered by the script. Locking value under P2SH is the
//! documented next step, requiring P2SH signature-script plumbing not yet in
//! `kcp-common`.
...
///
/// Change below this threshold is folded into the fee. Zero-value outputs
/// are non-standard ("dust") and small outputs risk the KIP-0009 storage-mass
/// limit (`mass ≈ 10¹²/amount` must stay ≤ 10⁶, requiring `amount ≥ 10⁶`).
pub const MIN_CHANGE_SOMPI: u64 = 1_000_000;

/// Default vault UTXO value in sompi (0.1 KAS).
///
/// Storage-mass at the 70-byte payload stays well within the KIP-0009 cap
/// (see module-level documentation for the calculation).
pub const DEFAULT_VAULT_VALUE_SOMPI: u64 = 10_000_000;
...
/// Anchor a vault evidence payload on-chain.
///
/// Selects a funding UTXO from `wallet`, builds a transaction with:
/// - output 0: `vault_value_sompi` locked to `wallet.address` (pay-to-address
///   in v0), carrying `payload_bytes` in the transaction payload field.
/// - output 1 (optional): change returned to `wallet.address` when
///   `(input - vault_value - fee) >= MIN_CHANGE_SOMPI`.
///
...
pub async fn anchor_vault_tx(
    client: &KaspaRpcClient,
    wallet: &Wallet,
    payload_bytes: Vec<u8>,
    vault_value_sompi: u64,
) -> Result<String> {
    let required = vault_value_sompi + CARRIER_FEE_SOMPI;
    ...
    let utxo = select_smallest_covering(entries, required.saturating_sub(1)).ok_or_else(|| {
        Error::Rpc(format!(
            "wallet {} has UTXOs but none above the required {} sompi \
             (vault_value {} + fee {})",
            wallet.address, required, vault_value_sompi, CARRIER_FEE_SOMPI
        ))
    })?;
    ...
    let change_amount = input_amount - vault_value_sompi - CARRIER_FEE_SOMPI;

    let input = TransactionInput::new(outpoint, vec![], 0, 0);
    let vault_output =
        TransactionOutput::new(vault_value_sompi, pay_to_address_script(&wallet.address));
    ...
    let mut outputs = vec![vault_output];
