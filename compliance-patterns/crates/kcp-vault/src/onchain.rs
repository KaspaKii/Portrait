...
//! - `lock_time < 500_000_000_000` → DAA score (use with `TimelockHeight`)
//! - `lock_time ≥ 500_000_000_000` → a **timestamp in Unix milliseconds**
//!   (use with `TimelockUnixSeconds`, whose name is a misnomer)
//!
//! The unit above the threshold is milliseconds, not seconds: consensus
//! compares `lock_time` against the block timestamp, and rusty-kaspa timestamps
//! come from `unix_now()`, which is `as_millis()`. A deadline given in real
//! unix seconds (~1.8e9) is therefore *below* the threshold and is treated as a
//! DAA score. The pure `crate::evaluator` compares the same deadline against a
//! seconds-valued `unix_seconds` field, so it disagrees with consensus about the
//! unit — see `KNOWN-ISSUES.md`.
//!
...
mod tests {
    ...
    /// scores; values at or above are timestamps in **Unix milliseconds**.
