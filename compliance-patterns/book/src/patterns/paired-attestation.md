...
**The v1 covenant is not custody.** CSFS signs a fixed 32-byte `msg_hash`, not
the transaction sighash, and the redeem script has no `OP_CHECKSIG` over the
spend — so the satisfier is a bearer credential. The moment a spend enters a
mempool both signatures are public in its signature script, and any observer
(trivially, any miner) can spend the same outpoint to themselves. Use it for
attestation signalling with dust-level value only, and never re-fund the
address. See the crate README's threat model and `KNOWN-ISSUES.md`.
