...
Two caveats on those previews. A deposit too small to mint a whole share is
**rejected** (`VaultError::ZeroSharesMinted`) rather than silently swallowed.
And when `total_shares == 0`, `preview_redeem` / `convert_to_assets` return a
virtual-offset extrapolation that can far exceed everything the vault holds —
it is **not** redeemable value. Conversions compute in `u128` and saturate at
`u64::MAX` rather than wrapping. See the crate README's threat model.
