...
- **On-chain enforcement** — the v1 covenant enforces L-1..L-4 at consensus via `validateOutputState` `[KCP-SL-002, KCP-SL-003]`. Note what it does *not* cover: the covenant state is `{lineage_id, seq, event_class, t_bucket, publisherPk}`, so the blinded **commitment is never bound by consensus** on any path, and `publisherPk` is unconstrained across a transition, so the publisher can rotate the authorised key at any append. See the crate README's threat model.
