...
## MEASURE THE BUDGET BEFORE FUNDING THE ADDRESS

255 is the largest value a `u8` `sigOpCount` can hold, so
`255 × 100_000 + 9_999 = 25,509,999` script units is the most a version-0 input
can ever commit. The reference proof shipped with the crate measures
**25,446,182 units — 0.25% under that ceiling.** A larger seal, a longer
control-inclusion path, or a different guest can cross it, and an over-budget
spend can never be submitted. The proof fields live inside the redeem script,
therefore inside the P2SH address, so there is no fallback spending path and
**the funds are permanently unrecoverable**.

Measure your own proof shape first (the helpers run the real consensus VM, so
they need the `wrpc` feature — the default build does not pull the engine):
...
```rust
let units = kcp_pq_anchor::sigop::measure_pq_anchor_units(&script)?;
assert!(kcp_pq_anchor::sigop::fits_pq_verify_budget(units)); // else: do not fund
```
...
4. **Measure the budget** (`--features wrpc`) and confirm it fits — see above
5. Submit the spending transaction with `sigOpCount = 255`
