...
// transferOwnership(newOwner) — onlyOwner-gated; Err(NotOwner) if `by` is not the owner
let rec = rec.transfer_ownership(owner_key, new_owner_key)?;

// renounceOwnership() — onlyOwner-gated
let rec = rec.renounce_ownership(new_owner_key)?;
```
