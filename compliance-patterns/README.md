...
of reusable covenant components. Every pattern crate ships tests and a
`## Threat model` section stating what consensus enforces, what it trusts
off-chain, and what it does not protect against. A gate test
(`crates/kcp-cli/tests/pattern_readme_threat_models.rs`) enforces that each
pattern crate *has* one, with its five fixed headings and the not-an-audit
stamp; the **content** is human-reviewed, not mechanically verified. The shared
plumbing crate `kcp-common` has no model of its own — its surface is covered by
the models of the patterns that use it. These are threat models, not audits:
...
452 Rust tests pass across the workspace on default features, and 533 with
`--all-features` (the extra 81 are the `wrpc`-gated real-engine and
budget-measurement tests); 2
ignored either way. Verified 2026-09-01, both feature sets, alongside
`cargo clippy --workspace --all-targets --all-features -- -D warnings`
clean and `cargo fmt --check` clean.
...
cargo test -p kcp-vault --features wrpc

# 3. Read the vault README to see what just ran
$EDITOR crates/kcp-vault/README.md
```

`--features wrpc` is required: the real-engine tests are feature-gated because
they pull the `rusty-kaspa` tree from git. Without it the engine module never
compiles — bare `cargo test -p kcp-vault` runs 54 tests, `--features wrpc` runs
82, and only the latter includes
`onchain::tests::multisig_2of2_lock_spend_executes_on_engine` (verified
2026-09-01).
