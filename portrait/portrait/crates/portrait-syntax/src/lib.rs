...
pub struct Entry {
    ...
    pub body: Vec<Stmt>,
    /// Explicit checked-model supply-change capability
    /// (`#[covenant(mode = …, supply_change = <field>)]`). When set, the named
    /// committed field is the authority that must be guaranteed to sign, and the
    /// entry is waived from value-conservation checking. This is a CHECKED-MODEL
    /// capability only — a UTXO covenant cannot inflate real coin.
...
pub enum Stmt {
    Require(Expr),
    Return(ReturnExpr),
    /// A `pays(index, payee, amount)` output-binding clause (B2): it binds the
    /// transaction output at `index` to pay the COMMITTED `amount` to the
    /// COMMITTED `payee`. `payee`/`amount` name committed state fields (or role
    /// params) — never spender-supplied args (validated in portrait-sema). It
    /// lowers to `Guard::OutputPays` in portrait-ir and to two
    /// output-introspection `require`s in portrait-emit.
    Pays {
        index: usize,
        payee: String,
        amount: String,
    },
    /// An `after(deadline)` consensus time-gate clause (B1): it binds the spend
    /// to be relayable only once the transaction's lock time reaches the COMMITTED
    /// `deadline`. The `deadline` is one of exactly two committed-time shapes (see
    /// [`AfterDeadline`]) — never a spender-supplied arg (validated in
    /// portrait-sema). It lowers to `Guard::TimeAtLeast` in portrait-ir and to a
    /// `require(tx.time >= prev_states[0].<deadline>)` in portrait-emit, which
    /// silverc routes to `OpCheckLockTimeVerify` — the single opcode that enforces
    /// BOTH the lock-time bound AND the non-final-sequence bundled check (defeating
    /// the final-sequence bypass).
    After {
...
/// The deadline of an `after(...)` consensus time-gate clause (B1). Exactly two
/// shapes are accepted (D1): a single committed time field, or a sum of two
/// committed time atoms (the `last_charged + period` window form). No arbitrary
/// arithmetic — the parser accepts only `ident` or `ident + ident`. Well-formedness
/// (both operands committed + time-named) is enforced in portrait-sema.
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum AfterDeadline {
    /// `after(field)` — the spend unlocks once tx lock time reaches the single
    /// committed time `field`.
    Field(String),
    /// `after(a + b)` — the spend unlocks once tx lock time reaches the SUM of two
    /// committed time atoms (e.g. `last_charged + period`).
    Sum(String, String),
}
...
pub enum Invariant {
    ValueConserved,
    NoUndeclaredState,
    /// A formula-bearing temporal invariant (A4-full):
    /// `invariant <name>: <entry> => after(<deadline>);`. It binds the named
    /// entrypoint `entry` to carry an `after(<deadline>)` consensus CLTV clause
    /// with exactly this `deadline`. This is a STRUCTURAL check (the checker
    /// verifies the entrypoint carries the matching consensus gate the emitter
    /// lowers to `OpCheckLockTimeVerify`) — NOT an SMT-discharged temporal
    /// obligation. It is a stronger, additional form of `temporal_guard`
    /// (existence-only), never a replacement.
    TemporalPath {
        name: String,
        entry: String,
        deadline: AfterDeadline,
...
impl<'a> Parser<'a> {
    ...
    fn parse_role(&mut self) -> Result<Role, String> {
        ...
        while !self.peek_symbol('}') {
            if self.peek_keyword("param") {
                ...
                let (mode, supply_change) = self.parse_covenant_attr()?;
                entrypoints.push(self.parse_entrypoint(mode, supply_change)?);
            } else if self.peek_keyword("entrypoint") {
                entrypoints.push(self.parse_entrypoint(CovenantMode::NonCovenant, None)?);
            } else {
    ...
    fn parse_covenant_attr(&mut self) -> Result<(CovenantMode, Option<String>), String> {
        self.expect_symbol('#')?;
        self.expect_symbol('[')?;
        ...
        // Optional `, supply_change = <field>`: the explicit checked-model
        // supply-change capability. The named field is the committed authority
        // that sema requires to be guaranteed to sign.
        let supply_change = if self.peek_symbol(',') {
            self.bump();
            self.expect_keyword("supply_change")?;
            self.expect_symbol('=')?;
            Some(self.expect_ident()?)
        } else {
            None
    ...
    fn parse_block(&mut self, mode: &CovenantMode) -> Result<Vec<Stmt>, String> {
        ...
        while !self.peek_symbol('}') {
            ...
            } else if self.peek_keyword("pays") && self.peek_symbol_at(1, '(') {
                self.bump();
                body.push(self.parse_pays_stmt()?);
            } else if self.peek_keyword("after") && self.peek_symbol_at(1, '(') {
                self.bump();
                body.push(self.parse_after_stmt()?);
    ...
    /// Parse `pays(<index>, <payee>, <amount>);` (B2). `index` is a non-negative
    /// integer literal naming the transaction output to bind; `payee` and
    /// `amount` are bare identifiers that must resolve to COMMITTED state (a role
    /// param or state field) — that requirement is enforced in portrait-sema, not
    /// here. Leading `pays` and the `(` are already confirmed by the caller.
    fn parse_pays_stmt(&mut self) -> Result<Stmt, String> {
        self.expect_symbol('(')?;
        let index = self.expect_output_index()?;
        self.expect_symbol(',')?;
        let payee = self.expect_ident()?;
        self.expect_symbol(',')?;
    ...
    /// Parse `after(<deadline>);` (B1). The deadline is one of exactly two shapes
    /// (D1): a bare identifier `after(field)`, or a sum of two identifiers
    /// `after(a + b)`. Each identifier must resolve to COMMITTED state (a role
    /// param or state field) carrying a conventional time name — that requirement
    /// is enforced in portrait-sema, not here. Leading `after` and the `(` are
    /// already confirmed by the caller.
    fn parse_after_stmt(&mut self) -> Result<Stmt, String> {
        let deadline = self.parse_after_deadline()?;
        self.expect_symbol(';')?;
        Ok(Stmt::After { deadline })
    }

    /// Read the `( <deadline> )` of an `after(...)` clause into an
    /// [`AfterDeadline`] (D1: `after(field)` or `after(a + b)`). Shared by the
    /// statement form (`parse_after_stmt`) and the formula-bearing temporal
    /// invariant surface (`invariant <name>: <entry> => after(<deadline>);`) so
    /// the invariant binds to exactly the construct the emitter lowers to
    /// `OpCheckLockTimeVerify`. Leading `after` is confirmed by the caller.
    fn parse_after_deadline(&mut self) -> Result<AfterDeadline, String> {
        self.expect_symbol('(')?;
        let first = self.expect_ident()?;
        let deadline = if self.peek_symbol('+') {
            self.expect_symbol('+')?;
            let second = self.expect_ident()?;
            AfterDeadline::Sum(first, second)
        } else {
            AfterDeadline::Field(first)
        };
        self.expect_symbol(')')?;
        Ok(deadline)
    }

    /// Parse a non-negative integer literal into the `usize` output index a
    /// `pays(...)` clause carries.
    fn expect_output_index(&mut self) -> Result<usize, String> {
        match self.bump() {
            Some(Token {
                ...
                self.error_at(
                    start,
                    "pays(...) output index must be a non-negative integer",
                )
            }),
            Some(tok) => Err(self.error_at(
                tok.start,
                "pays(...) output index must be an integer literal",
            )),
    ...
    fn parse_invariant(&mut self) -> Result<Invariant, String> {
        ...
        let name = self.expect_ident()?;
        // A4-full: `invariant <name>: <entry> => after(<deadline>);` — the
        // formula-bearing temporal form. A bare `invariant <name>;` (no colon)
        // stays the existing predicate-name form, unchanged.
        if self.peek_symbol(':') {
            self.expect_symbol(':')?;
            let entry = self.expect_ident()?;
            ...
            let deadline = self.parse_after_deadline()?;
            self.expect_symbol(';')?;
            return Ok(Invariant::TemporalPath {
                name,
                entry,
                deadline,
            });
    ...
    /// Render a parse diagnostic located at byte offset `pos`. Every one of the
    /// parser's rejections funnels through here, so the rendering upgrade lands on
    /// all of them at once: `line:col` plus the offending source line and a caret
    /// under the offending column. A raw byte offset is not something an author can
    /// act on without counting bytes; a caret is.
    fn error_at(&self, pos: usize, msg: &str) -> String {
        let (line, col) = line_col(self.src, pos);
        let text = self.src.lines().nth(line - 1).unwrap_or("");
        format!(
...
/// 1-based (line, column) of byte offset `pos` in `src`, counting columns in
/// CHARACTERS (not bytes) so the caret lands under the offending character in a
/// source containing multi-byte text. A `pos` at or past the end of `src` reports
...
"#;
    ...
    #[test]
    fn pays_clause_parses_to_output_binding_stmt() {
        // A `pays(index, payee, amount)` clause in a covenant transition body
        // parses to a structured `Stmt::Pays`, not a `Stmt::Raw` hole.
        let src = r#"
...
"#;
        let program = parse(src).expect("pays clause should parse");
        let body = first_entry_body(&program);
        assert!(
            !body.iter().any(|s| matches!(s, Stmt::Raw(_))),
            "pays must not fall back to Raw"
        );
        let pays = body
            .iter()
            .find_map(|s| match s {
                Stmt::Pays {
                    ...
                    payee,
                    amount,
                } => Some((*index, payee.clone(), amount.clone())),
                _ => None,
            })
            .expect("body has a pays clause");
        assert_eq!(pays, (0usize, "seller".to_string(), "amount".to_string()));
    }

    #[test]
    fn after_clause_parses_to_time_gate_stmt() {
        // An `after(deadline)` clause in a covenant transition body parses to a
        // structured `Stmt::After`, not a `Stmt::Raw` hole.
        let src = r#"
pragma portrait ^0.1.0;
app Vault {
  role vault {
...
"#;
        let program = parse(src).expect("after clause should parse");
        let body = first_entry_body(&program);
        assert!(
            !body.iter().any(|s| matches!(s, Stmt::Raw(_))),
            "after must not fall back to Raw"
        );
        let deadline = body
            .iter()
            .find_map(|s| match s {
                Stmt::After { deadline } => Some(deadline.clone()),
                _ => None,
            })
            .expect("body has an after clause");
        assert_eq!(deadline, AfterDeadline::Field("unlock_bucket".to_string()));
    }

    #[test]
    fn after_sum_clause_parses_to_sum_deadline() {
        // `after(a + b)` (D1) parses to `AfterDeadline::Sum`, the two-atom window
        // form; a single `after(a)` stays `AfterDeadline::Field` (unchanged).
        let src = r#"
pragma portrait ^0.1.0;
app Sub {
  role sub {
...
"#;
        let program = parse(src).expect("after-sum clause should parse");
        let body = first_entry_body(&program);
        let deadline = body
            .iter()
            .find_map(|s| match s {
        ...
        assert_eq!(
            deadline,
            AfterDeadline::Sum("last_charged".to_string(), "period".to_string())
        );
    }

    #[test]
    fn formula_bearing_invariant_parses() {
        // A4-full: `invariant <name>: <entry> => after(<deadline>);` parses to a
        // `TemporalPath` carrying the entry ident and the reused `AfterDeadline`.
        let src = r#"
pragma portrait ^0.1.0;
app Vault {
  role vault {
...
"#;
        let program = parse(src).expect("formula-bearing invariant should parse");
        let inv = program
            .app
            .invariants
            .iter()
            .find_map(|i| match i {
                Invariant::TemporalPath {
                    name,
                    entry,
                    deadline,
                } => Some((name.clone(), entry.clone(), deadline.clone())),
                _ => None,
            })
            .expect("a TemporalPath invariant");
        assert_eq!(
            inv,
            (
                "refund_after_deadline".to_string(),
                "refund".to_string(),
                AfterDeadline::Field("deadline".to_string())
            )
        );
    }

    #[test]
    fn bare_invariant_still_parses() {
        // Regression: a bare `invariant <name>;` (no colon) keeps the existing
        // predicate-name form, unchanged by A4-full.
        let src = r#"
pragma portrait ^0.1.0;
app Vault {
  role vault {
    ...
    state { pubkey owner; }
    #[covenant(mode = transition)]
    entrypoint function poke(sig auth) : (pubkey owner) {
      requires checkSig(auth, owner);
      return Vault { owner: owner };
    }
  }
  lifecycle { live -> live via vault.poke; }
  invariant value_conserved;
  invariant temporal_guard;
  invariant no_undeclared_state;
}
"#;
        let program = parse(src).expect("bare invariants should parse");
        assert!(matches!(
            program.app.invariants.as_slice(),
            [
                Invariant::ValueConserved,
                Invariant::Custom(g),
                Invariant::NoUndeclaredState,
            ] if g == "temporal_guard"
        ));
    }
...
"#
    ...
    fn parse_error_reports_line_and_column() {
        // A raw byte offset is not actionable without counting bytes. Every parser
        // rejection funnels through `error_at`, so the location must read as
        // `line:col` of the offending token.
        let src = "pragma portrait ^0.1.0;\napp F {\n  role r {\n    param int 7bad;\n  }\n}\n";
        let err = parse(src).expect_err("a numeric param name must not parse");
        assert!(
            err.contains("at line 4:15"),
            "error must locate the offending token by line:col, got: {err}"
        );
        assert!(
            !err.contains("at byte"),
            "the raw byte-offset rendering must be gone, got: {err}"
        );
    }

    #[test]
    fn parse_error_renders_the_offending_source_line_with_a_caret() {
        // The diagnostic must show the source line and point at the column, so the
        // author can see the rejection without opening an editor at an offset.
        let src = "pragma portrait ^0.1.0;\napp F {\n  role r {\n    param int 7bad;\n  }\n}\n";
        let err = parse(src).expect_err("a numeric param name must not parse");
        ...
        assert_eq!(
            lines.len(),
            3,
            "expected message + source line + caret: {err}"
        );
        assert_eq!(
            lines[1], "      param int 7bad;",
            "source line missing: {err}"
        );
        assert_eq!(
            lines[2],
            format!("  {}^", " ".repeat(14)),
            "caret must sit under the offending column: {err}"
        );
    }
