...
fn mint_entrypoint_emits_no_conservation_vc() {
    // An entry carrying the explicit `supply_change = owner` capability is an
    // authorised supply change (parity with sema) — no conservation VC, so it is
    // never spuriously REFUTED. The annotation, never the name, is what exempts.
...
#[test]
fn unannotated_mint_entrypoint_emits_conservation_vc() {
    // Companion to the above: a `mint`-named entry WITHOUT the `supply_change`
    // annotation is now conservation-CHECKED — Lens emits a ValueConservation VC
    // over its internal split, agreeing with sema that the name buys nothing.
    // (The old name-exemption would have suppressed this VC.)
    let src = r#"pragma portrait ^0.1.0;
app Minter {
  role token {
    param int    pool_a_balance;
    param int    pool_b_balance;
    param pubkey owner;
    state { int pool_a_balance; int pool_b_balance; pubkey owner; }
    #[covenant(mode = transition)]
    entrypoint function mint_rebalance(sig auth, int x) : (int pool_a_balance, int pool_b_balance, pubkey owner) {
      requires checkSig(auth, owner);
      requires x >= 0;
      requires x <= pool_a_balance;
      return Minter { pool_a_balance: pool_a_balance - x, pool_b_balance: pool_b_balance + x, owner: owner };
    }
  }
  lifecycle { live -> live via token.mint_rebalance; }
  invariant no_undeclared_state;
}
"#;
    let prog = checked_program(src);
    ...
    assert!(
        reports
            .iter()
            .any(|r| r.vc_kind == VcKind::ValueConservation),
        "an unannotated mint must now be conservation-checked (Lens agrees with sema)"
    );
}
