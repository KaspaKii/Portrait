...
pub fn build_conservation_vc(role: &Role, entry: &Entry) -> Result<crate::VcReport, LensError> {
    ...
    // Supply-change exemption (parity with sema): an entry carrying the explicit
    // `#[covenant(mode = …, supply_change = A)]` capability has no conservation
    // VC. Sema remains the single source of the authority-signs check; Lens only
    // agrees on WHICH entries are exempt (the annotation, never the name).
    if entry.supply_change.is_some() {
        return Err(LensError::Unsupported(format!(
            "{} carries an explicit supply_change capability (exempt from conservation)",
            entry.name
        )));
    }
