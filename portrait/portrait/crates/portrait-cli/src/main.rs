...
fn main() {
    ...
    match cmd {
        ...
        "atelier-build" => cmd_atelier_build(&args[2..]),
...
fn render_explain(
    ...
    for inv in &program.app.invariants {
        ...
        // Honest scope note where a check's guarantee is narrower than its name
        // suggests (M-2). `temporal_guard` is existence-only.
        if name == "temporal_guard" {
            out.push_str(
                "        note: a committed-time gate EXISTS on a mutating transition \
                 (existence-only); does NOT bind every value-moving path to a deadline \
                 (formula-bearing invariant is the tracked follow-up)\n",
            );
        }
        // A4-full: the formula-bearing temporal invariant is a STRUCTURAL check —
        // it verifies the named entrypoint carries the matching consensus CLTV
        // clause the emitter lowers to `OpCheckLockTimeVerify`. It is NOT an
        // SMT-discharged temporal obligation.
        if let Invariant::TemporalPath { entry, .. } = inv {
            out.push_str(&format!(
                "        note: the named entrypoint `{entry}` carries the matching \
                 consensus CLTV gate the emitter lowers to OpCheckLockTimeVerify \
                 (STRUCTURAL check); NOT an SMT-discharged temporal obligation\n",
            ));
        }
        // A6: payout_bound coverage signal (H-1) — print how many settling
        // transitions the recognizer matched, so a vacuous (0-match) or partial
        // pass is visible at the point of use. STRUCTURAL, existence-only scope.
        if name == "payout_bound" {
            let n = portrait_sema::settling_transition_count(&program.app);
            out.push_str(&format!(
                "        note: payout_bound: {n} settling transition(s) recognized, each \
                 required to carry `pays(...)` (STRUCTURAL existence check that a recognized \
                 one-shot-flag settlement binds a committed output); does NOT verify the pays \
                 binds THIS settlement's own coin/payee, and is NOT a value-conservation proof\n",
            ));
        }
    }
...
fn run_check(path: &str) -> Result<(), String> {
    ...
    // M5: `check` is the documented first gate and the Hallmark cites it, so it
    // must cover the genesis-binding rule too. That rule cannot live in
    // `portrait_sema::check` — many parse/check-only fixtures declare state fields
    // with no params at all — so `check` runs the emit-side validator as an
    // additional pass over the projected models. `portrait_sema::check`'s own
    // contract is unchanged.
    let cartoon = portrait_ir::lower(&program);
    for model in portrait_project::project(&cartoon) {
        portrait_emit::validate_genesis_binding(&model)?;
    }
    for w in portrait_sema::warnings(&program) {
        eprintln!("{}", w);
    }
    Ok(())
}
...
fn run_build(path: &str) -> Result<Vec<String>, String> {
    ...
    // Same warning surface as `check` and `engrave`: a declared conservation
    // invariant that matches no value-bearing field must not pass in silence.
    for w in portrait_sema::warnings(&program) {
        eprintln!("{}", w);
    }
    let cartoon = portrait_ir::lower(&program);
    ...
    for (model, sil) in models.iter().zip(sil_files.iter()) {
        ...
        if let Some(w) = portrait_emit::placeholder_ctor_warning(model) {
            eprintln!("{}", w);
        }
...
fn run_engrave(path: &str) -> Result<(), String> {
    ...
    for (model, sil) in models.iter().zip(sil_files.iter()) {
        ...
        // H2: the CTOR JSON is an all-zero placeholder — say so, loudly, every
        // time. Silence here is what let `portrait new && portrait ship` print a
        // KovId for a zero-genesis instance as if it were a deployment.
        if let Some(w) = portrait_emit::placeholder_ctor_warning(model) {
            eprintln!("{}", w);
        }
...
fn cmd_atelier_build(args: &[String]) {
    // Parse the optional `--allow-unimplemented-vprog` flag and one positional
    // file. Plain `portrait atelier-build <file>` is byte-identical to before,
    // except the emitted guest now REFUSES TO BUILD until its predicate is
    // authored (see run_atelier_build). The flag opts into a loudly-marked
    // true-returning placeholder instead.
    let mut allow_unimplemented = false;
    let mut path: Option<&str> = None;
    for a in args {
        match a.as_str() {
            "--allow-unimplemented-vprog" => allow_unimplemented = true,
            other if other.starts_with('-') => {
                eprintln!("error: unknown flag {}", other);
                exit(2);
            }
    ...
    let Some(p) = path else {
        eprintln!("usage: portrait atelier-build [--allow-unimplemented-vprog] <file>");
        exit(2);
    };
    match run_atelier_build(p, allow_unimplemented) {
...
fn run_atelier_build(path: &str, allow_unimplemented: bool) -> Result<String, String> {
    let src_path = std::path::Path::new(path);
    let out_dir = src_path.parent().unwrap_or(std::path::Path::new("."));
    let src = std::fs::read_to_string(path).map_err(|e| e.to_string())?;
    let program = portrait_syntax::parse(&src)?;
    ...
    let file = portrait_atelier::emit_guest_main(&cartoon, kov_id, allow_unimplemented)
        .ok_or_else(|| "no VProg transitions in this portrait file".to_string())?;

    if allow_unimplemented {
        println!(
            "[atelier] WARNING: emitted with --allow-unimplemented-vprog — the guest's \
             predicate() is a placeholder that returns true and proves NOTHING."
        );
    } else {
        println!(
            "[atelier] NOTE: the emitted guest's predicate() is an unimplemented stub and \
             will REFUSE TO BUILD (compile_error!). Author the real verification logic, or \
             re-run with --allow-unimplemented-vprog for a loudly-marked placeholder."
        );
    }
...
fn render_ship_summary(
    ...
    if let Some(k) = kov_id {
        ...
        // H2: the KovId is derived from the compiled covenant bytes, which were
        // compiled against the ALL-ZERO placeholder ctor. Never let it read as a
        // deployment identity.
        if hm.genesis.as_deref() == Some(portrait_emit::GENESIS_PLACEHOLDER_ZERO) {
            out.push_str(
                "         ^ PLACEHOLDER instance — built from the all-zero constructor args in \
                 <Name>_ctor.json, NOT your deployment. Supply real constructor args before \
                 deploying.\n",
            );
        }
    }
...
/// `unknown` — never a false `proved`. Default exit code is non-zero only on a
/// `refuted` VC or a hard Lens refusal; `unknown` and z3-absent exit 0 (UNKNOWN
/// is first-class, not a failure).
///
/// STRICT (`--strict`, opt-in): additionally `exit(1)` when the prove run does
/// not meet the strict bar — zero VCs generated, the solver is missing, any
/// entrypoint is covered by no VC, or any VC fell to UNKNOWN. Default behaviour
/// (no `--strict`) is unchanged for back-compat.
fn cmd_prove(args: &[String]) {
    ...
    let Some(p) = path else {
        eprintln!("usage: portrait prove <file> [--strict]");
        exit(2);
    };
    ...
    // Every covenant *transition* entrypoint the covenant declares — the ground
    // truth against which VC coverage is measured (an entrypoint with no VC is
    // `unchecked`, and a strict failure).
    let entrypoints: Vec<String> = program
        .app
        .roles
        .iter()
        .flat_map(|role| {
            role.entrypoints
                .iter()
                .filter(|e| matches!(e.mode, portrait_syntax::CovenantMode::Transition))
                .map(move |e| format!("{}.{}", role.name, e.name))
        })
        .collect();

    let (summary, any_refuted) = render_prove_summary(p, &reports, z3_present, &entrypoints);
    print!("{}", summary);

    // Honest exit code: non-zero on a refuted VC always. In strict mode, ALSO
    // non-zero when the run fails to meet the strict bar. Default (non-strict)
    // exits 0 on UNKNOWN / z3-absent / zero-VC for back-compat.
    let mut fail = any_refuted;
    if strict {
        let reasons = strict_prove_failures(&entrypoints, &reports, z3_present);
        if !reasons.is_empty() {
            eprintln!("strict: prove did not meet the strict bar:");
...
/// The strict-mode failure reasons for a prove run (pure; unit-testable). Empty
/// ⇒ the run met the strict bar. A reason is added for: zero VCs generated, an
/// absent solver, any UNKNOWN / undischarged outcome, and any declared
/// transition entrypoint that no VC covers.
fn strict_prove_failures(
    entrypoints: &[String],
    ...
    if reports.is_empty() {
        reasons.push("no VCs were generated (strict requires at least one)".to_string());
    }
    if !z3_present {
        reasons.push("the z3 solver is not available (strict requires a solver)".to_string());
    }
    for r in reports {
        match &r.outcome {
            Some(Outcome::Unknown { reason }) => reasons.push(format!(
                "{} {} fell to UNKNOWN: {}",
                r.entrypoint,
                r.vc_kind.label(),
                reason
    ...
    for ep in entrypoints {
        if !reports.iter().any(|r| &r.entrypoint == ep) {
            reasons.push(format!("entrypoint {} is covered by no VC", ep));
        }
    }
    reasons
}

/// Render the coverage matrix (entrypoint × property → proved / unknown / exempt
/// / unchecked) for the prove run. Pure; unit-testable. An entrypoint with no VC
/// at all is `unchecked`; a property class with no VC on an otherwise-covered
/// entrypoint is `exempt`; otherwise the cell reflects the solver outcome.
fn render_coverage_matrix(entrypoints: &[String], reports: &[portrait_lens::VcReport]) -> String {
    use portrait_lens::{Outcome, VcKind};
    const PROPERTIES: [VcKind; 5] = [
        VcKind::ValueConservation,
        VcKind::Spend,
        VcKind::Range,
        VcKind::Refinement,
        VcKind::InvariantPreservation,
    ];
    let mut out = String::from(
        "  coverage matrix (entrypoint × property → proved/unknown/exempt/unchecked):\n",
    );
    for ep in entrypoints {
        let ep_reports: Vec<&portrait_lens::VcReport> =
            reports.iter().filter(|r| &r.entrypoint == ep).collect();
        if ep_reports.is_empty() {
            out.push_str(&format!("    {}: unchecked (no VC generated)\n", ep));
            continue;
        }
        let cells: Vec<String> = PROPERTIES
            .iter()
            .map(|kind| {
                let status = match ep_reports.iter().find(|r| r.vc_kind == *kind) {
                    None => "exempt",
                    Some(r) => match &r.outcome {
                        Some(Outcome::Proved { .. }) => "proved",
                        Some(Outcome::Refuted { .. }) => "refuted",
                        Some(Outcome::Unknown { .. }) | None => "unknown",
                    },
                };
                format!("{}: {}", kind.label(), status)
            })
            .collect();
        out.push_str(&format!("    {}: {}\n", ep, cells.join(", ")));
    }
...
fn render_prove_summary(
    ...
    for r in reports {
        ...
        match &r.outcome {
            Some(Outcome::Proved { unsat_core }) => {
                out.push_str(&format!(
                    "  [proved] {} {} [MODEL-ONLY; not the emitted .sil] — negated VC is unsat (cross-checked: confirmed \
    ...
    // Coverage matrix, ABOVE the footer, so a reader sees which entrypoints /
    // properties are actually discharged (vs exempt / unchecked).
    out.push_str(&render_coverage_matrix(entrypoints, reports));
...
/// CORRESPONDS.
///
/// The headline verdict carries an inline scope suffix so a CORRESPONDS cannot be
/// misread as behavioural-equivalence or a `.sil` audit; the full structural
/// caveat still follows in [`portrait_lens::TRANSLATION_STRUCTURAL_FOOTER`].
const CORRESPONDS_LINE: &str = "CORRESPONDS — every model guard maps to a .sil require, and model and .sil agree on every value-bearing write (none added, altered, or dropped) [STRUCTURAL; not behavioural equivalence]";
...
mod tests {
    ...
    #[test]
    fn render_explain_notes_temporal_path_structural_scope() {
        // A4-full: the formula-bearing temporal invariant renders an honest scope
        // note — the named entrypoint CARRIES the matching consensus CLTV gate
        // (STRUCTURAL), NOT an SMT-discharged temporal obligation.
        let src = r#"
...
"#;
        let program = portrait_syntax::parse(src).expect("parse");
        let result = portrait_sema::check(&program);
        assert!(result.is_ok(), "temporal-path app should pass sema");
        let report = render_explain(&program, &result);
        assert!(report.contains("[ok] refund_after_deadline (declared)"));
        assert!(report.contains("the named entrypoint `refund` carries the matching"));
        assert!(report.contains("STRUCTURAL check"));
        assert!(report.contains("NOT an SMT-discharged temporal obligation"));
        assert!(report.contains("OpCheckLockTimeVerify"));
    }

    #[test]
    fn render_explain_notes_payout_bound_match_count() {
        // A6/H-1: the payout_bound note renders the recognized-settlement count so
        // coverage is visible at the point of use (STRUCTURAL, existence-only).
        let src = r#"
pragma portrait ^0.1.0;
app Escrow {
  role escrow {
...
"#;
        let program = portrait_syntax::parse(src).expect("parse");
        let result = portrait_sema::check(&program);
        assert!(result.is_ok(), "payout_bound app should pass sema");
        let report = render_explain(&program, &result);
        assert!(report.contains("[ok] payout_bound (declared)"));
        assert!(report.contains("payout_bound: 1 settling transition(s) recognized"));
        assert!(report.contains("does NOT verify the pays binds THIS settlement's own coin/payee"));
    }
...
"#;
    ...
    #[test]
    fn render_explain_prints_temporal_guard_honest_limit() {
        // M-2: the explain surface must state temporal_guard's existence-only
        // scope, not just `[ok] temporal_guard (declared)`.
...
"#;
        ...
        assert!(
            result.is_ok(),
            "temporal source should pass sema: {result:?}"
        );
        let report = render_explain(&program, &result);
        assert!(report.contains("[ok] temporal_guard (declared)"));
        assert!(
            report.contains("existence-only")
                && report.contains("does NOT bind every value-moving path"),
            "explain must carry the honest temporal_guard scope note, got: {report}"
        );
    }
    ...
    fn render_prove_summary_proved_line_and_always_carries_the_boundary() {
        ...
        let (summary, any_refuted) = render_prove_summary(
            "X.portrait",
            &reports,
            true,
            &["pool.rebalance".to_string()],
        );
        assert!(!any_refuted);
        assert!(summary.contains("[proved] pool.rebalance value-conservation"));
        // The proved headline carries the inline MODEL-ONLY scope suffix so it
        // cannot be misread as a proof of the emitted .sil.
        assert!(summary.contains("[MODEL-ONLY; not the emitted .sil]"));
    ...
    fn render_prove_summary_crosscheck_does_not_overclaim_independence() {
        ...
        let (summary, _) =
            render_prove_summary("X.portrait", &reports, true, &["vault.spend".to_string()]);
    ...
    // ===== prove: strict mode + coverage matrix + scope suffixes =============

    fn vc(
        entrypoint: &str,
        kind: portrait_lens::VcKind,
        outcome: portrait_lens::Outcome,
    ) -> portrait_lens::VcReport {
        portrait_lens::VcReport {
    ...
    #[test]
    fn strict_prove_fails_on_empty_reports() {
        // No VCs generated: default prove exits 0, but --strict must fail.
        let reasons = strict_prove_failures(&[], &[], true);
        assert!(
            reasons.iter().any(|r| r.contains("no VCs were generated")),
            "empty reports must fail strict; got: {reasons:?}"
        );
    }

    #[test]
    fn strict_prove_fails_on_all_unknown() {
        use portrait_lens::{Outcome, VcKind};
        let reports = vec![vc(
            "pool.rebalance",
            VcKind::ValueConservation,
            Outcome::Unknown {
                reason: "outside decidable fragment".to_string(),
            },
        )];
        let reasons = strict_prove_failures(&["pool.rebalance".to_string()], &reports, true);
        assert!(
            reasons.iter().any(|r| r.contains("fell to UNKNOWN")),
            "an UNKNOWN outcome must fail strict; got: {reasons:?}"
        );
    }

    #[test]
    fn strict_prove_fails_when_z3_absent() {
        use portrait_lens::{Outcome, VcKind};
        let reports = vec![vc(
            "pool.rebalance",
            VcKind::ValueConservation,
            Outcome::Proved { unsat_core: vec![] },
        )];
        let reasons = strict_prove_failures(&["pool.rebalance".to_string()], &reports, false);
        assert!(
            reasons
                .iter()
                .any(|r| r.contains("z3 solver is not available")),
            "an absent solver must fail strict; got: {reasons:?}"
        );
    }

    #[test]
    fn strict_prove_fails_on_uncovered_entrypoint() {
        use portrait_lens::{Outcome, VcKind};
        let reports = vec![vc(
            "vault.release",
            VcKind::ValueConservation,
        ...
        // `vault.claw` has no VC → strict must flag it.
        let reasons = strict_prove_failures(
            &["vault.release".to_string(), "vault.claw".to_string()],
            &reports,
            true,
        );
        assert!(
            reasons
                .iter()
                .any(|r| r.contains("vault.claw is covered by no VC")),
            "an uncovered entrypoint must fail strict; got: {reasons:?}"
        );
    }

    #[test]
    fn strict_prove_passes_when_fully_proved() {
        use portrait_lens::{Outcome, VcKind};
        let reports = vec![vc(
            "vault.release",
            VcKind::ValueConservation,
        ...
        let reasons = strict_prove_failures(&["vault.release".to_string()], &reports, true);
        assert!(
            reasons.is_empty(),
            "a fully-proved run must pass strict; got: {reasons:?}"
        );
    }

    #[test]
    fn non_strict_summary_exits_zero_on_empty_reports() {
        // Back-compat: the pure summary never flags failure for zero VCs; the
        // note is printed and any_refuted stays false (default exit 0).
        let (summary, any_refuted) = render_prove_summary("X.portrait", &[], true, &[]);
        assert!(
            !any_refuted,
            "zero VCs must not flag failure in the default path"
        );
        assert!(summary.contains("no VCs generated"));
    }

    #[test]
    fn coverage_matrix_marks_proved_exempt_and_unchecked() {
        use portrait_lens::{Outcome, VcKind};
        let reports = vec![vc(
            "vault.release",
            VcKind::ValueConservation,
        ...
        assert!(matrix.contains("coverage matrix"));
        // Covered entrypoint: the proved property + an exempt property class.
        assert!(matrix.contains("vault.release: value-conservation: proved"));
        assert!(matrix.contains("range-overflow: exempt"));
        // Entrypoint with no VC at all is unchecked.
        assert!(matrix.contains("vault.claw: unchecked (no VC generated)"));
    }

    #[test]
    fn corresponds_line_carries_structural_scope_suffix() {
        assert!(CORRESPONDS_LINE.contains("CORRESPONDS"));
        assert!(CORRESPONDS_LINE.contains("[STRUCTURAL; not behavioural equivalence]"));
    }
