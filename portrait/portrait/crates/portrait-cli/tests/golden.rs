...
fn reject_payroll_weakens_temporal_guard() {
    // A4 (external review) — `temporal_guard` guarantees a committed-time gate
    // EXISTS on a mutating transition (role-level existence). PayrollStream now
    // carries TWO independent committed-time gates on `release`: the caller-asserted
    // `now_bucket >= last_paid + period` (which also anchors `last_paid` forward)
    // AND the consensus `after(last_paid + period)` CLTV clause. Either alone
    // satisfies the invariant, so a single-line edit no longer bites — this
    // mutation weakens BOTH gates in one surgical edit (collapsing them to a single
    // non-committed-time compare against the spender arg `amount`). After the edit
    // NO mutating transition asserts a committed-time gate, so the role-level check
    // rejects, naming `temporal_guard`.
    reject_mutation(
        "payroll:temporal_guard",
        PAYROLL_STREAM_SRC,
        "requires now_bucket >= last_paid + period;       // rate limit: one release per period (temporal_guard)\n      after(last_paid + period);                       // SCRIPT-ENFORCED (B1): consensus CLTV gate on\n                                                       // the committed window sum (OpCheckLockTimeVerify)",
...
// ---------------------------------------------------------------------------
//  EXTERNAL-REVIEW FIXTURES — authorization/soundness bypass vectors (A1–A4).
//
//  Seven hand-written sources pinning the guard-aware authorization checker: one
//  honest control (00), one caller-supplied-key control (01), and five bypass
//  shapes the pre-hardening checker accepted (02–06). Each drives the SAME
//  library front-end the goldens use (`portrait_syntax::parse` +
//  `portrait_sema::check`), asserting Ok/Err on the checker verdict only.
// ---------------------------------------------------------------------------

/// Parse + check, asserting the checker ACCEPTS.
fn fixture_accepts(label: &str, src: &str) {
    let program =
        portrait_syntax::parse(src).unwrap_or_else(|e| panic!("[{label}] must parse: {e}"));
    portrait_sema::check(&program).unwrap_or_else(|ds| {
        let msgs: Vec<_> = ds.into_iter().map(|d| d.message).collect();
        panic!(
            "[{label}] expected acceptance, rejected: {}",
            msgs.join("; ")
        );
    });
}

/// Parse (must succeed — the defect is semantic) + check, asserting the checker
/// REJECTS with a diagnostic naming `needle`.
fn fixture_rejects(label: &str, src: &str, needle: &str) {
    let program =
        portrait_syntax::parse(src).unwrap_or_else(|e| panic!("[{label}] must parse: {e}"));
    let diags = portrait_sema::check(&program).expect_err(&format!(
        "[{label}] expected rejection, but it was accepted"
    ));
    let joined = diags
        .into_iter()
        .map(|d| d.message)
    ...
    assert!(
        ...
        "[{label}] rejection must name `{needle}`, got: {joined}"
    );
}

const FIX_00_BASELINE_GOOD: &str = r#"
pragma portrait ^0.1.0;
app A {
  role r {
    param pubkey owner;
    param sig owner_auth;
    state { pubkey owner; int balance; }
    #[covenant(mode = transition)]
    entrypoint function withdraw(int amount) : (pubkey owner, int balance) {
      requires checkSig(owner_auth, owner);
...
const FIX_01_BASELINE_BAD: &str = r#"
pragma portrait ^0.1.0;
app A {
  role r {
    param pubkey owner;
    state { pubkey owner; int balance; }
    #[covenant(mode = transition)]
    entrypoint function withdraw(sig auth, pubkey who, int amount) : (pubkey owner, int balance) {
...
const FIX_04_MINT_BYPASS: &str = r#"
pragma portrait ^0.1.0;
app A {
  role r {
    param int start;
    state { int balance; }
    #[covenant(mode = transition)]
    entrypoint function mintDrain() : (int balance) {
      return A { balance: 999999999 };
    }
  }
...
const FIX_05_FAKE_MULTISIG: &str = r#"
pragma portrait ^0.1.0;
app A {
  role t {
    param pubkey signer_a;
    param pubkey signer_b;
    ...
    entrypoint function spend(sig auth_a, sig auth_b, int amount) : (pubkey signer_a, pubkey signer_b, int balance) {
      requires checkSig(auth_a, signer_a) || checkSig(auth_b, signer_b);
...
const FIX_06_TEMPORAL_REMOVED: &str = r#"
pragma portrait ^0.1.0;
app A {
  role r {
    param pubkey owner;
    param int genesis_deadline;
...
/// Control: the honest single-owner covenant is accepted.
#[test]
fn fixture_00_baseline_good_accepts() {
    fixture_accepts("00_baseline_good", FIX_00_BASELINE_GOOD);
}

/// Control: authorizing against a caller-supplied (non-committed) key is rejected.
#[test]
fn fixture_01_baseline_bad_rejects() {
    fixture_rejects(
        "01_baseline_bad",
        FIX_01_BASELINE_BAD,
...
/// A1: `checkSig(owner) || checkSig(attacker)` — the attacker arm binds a
/// caller-supplied key, so no committed key is guaranteed to sign → rejected.
#[test]
fn fixture_02_or_bypass_rejects() {
    fixture_rejects("02_or_bypass", FIX_02_OR_BYPASS, "capability check failed");
}

/// A1: `!checkSig(auth, owner)` authorizes nothing → rejected.
#[test]
fn fixture_03_not_bypass_rejects() {
    fixture_rejects(
        "03_not_bypass",
        FIX_03_NOT_BYPASS,
        "capability check failed",
...
/// A2: a mint-named supply change with ZERO authorization under `value_conserved`
/// (C1-exempt from conservation, but the no-auth fail-safe still fires) → rejected.
#[test]
fn fixture_04_mint_bypass_rejects() {
    fixture_rejects(
        "04_mint_bypass",
        FIX_04_MINT_BYPASS,
        "capability check failed",
...
/// A3: a 1-of-2 `||` masquerading as multisig — only one committed key is
/// guaranteed on any path → `multisig_threshold` violated.
#[test]
fn fixture_05_fake_multisig_rejects() {
    fixture_rejects(
        "05_fake_multisig",
        FIX_05_FAKE_MULTISIG,
        "multisig_threshold",
    );
}

/// A4: `temporal_guard` declared but no mutating transition gates on committed
/// time (the only time comparison was removed) → rejected.
#[test]
fn fixture_06_temporal_removed_rejects() {
    fixture_rejects(
        "06_temporal_removed",
        FIX_06_TEMPORAL_REMOVED,
        "temporal_guard",
    );
}
