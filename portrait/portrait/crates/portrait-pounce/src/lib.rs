...
//! - `Mode::Transition` with `to = None`    → Covenant (B3 TERMINAL release — a
//!   lifecycle-ending covenant function that binds its payout via `pays(...)` and
//!   consumes the UTXO; `portrait-emit` emits it as a covenant, so it is NOT a vProg)
...
fn classify(tr: &Transition) -> Layer {
    match tr.mode {
        // Every `Mode::Transition` is a covenant function — INCLUDING a B3 TERMINAL
        // transition (`to = None`), which `portrait-emit` emits as a `binding = auth`
        // covenant release (it gates only on `!NonCovenant`, not on `to`). Mirror
        // that gate here: a terminal transition is a CONSENSUS covenant spend, not a
        // vProg. Classifying it VProg would make Atelier emit spurious ZK guests for a
        // covenant spend and make `derive_kov_id`'s `has_covenant` go false for a
        // terminal-only covenant. Only an explicitly off-chain `NonCovenant` is a vProg.
        Mode::Transition | Mode::Verification => Layer::Covenant,
...
mod tests {
    ...
    fn mixed_allocations() {
        ...
        assert_eq!(result.allocations[1].layer, Layer::VProg); // offchain (NonCovenant)
                                                               // B3: `burn` is a terminal Mode::Transition — a covenant release, NOT a vProg.
        assert_eq!(result.allocations[2].layer, Layer::Covenant);
    }

    #[test]
    fn terminal_mode_transition_is_covenant_not_vprog() {
        // R1: a TERMINAL transition (`Mode::Transition`, `to = None`) is a B3
        // lifecycle-ending covenant release that `portrait-emit` emits as a covenant
        // function. It must classify as Covenant, so Atelier does NOT emit a spurious
        // ZK guest for it and `derive_kov_id` still sees a covenant.
        let cartoon = make_cartoon(vec![make_transition("release", Mode::Transition, None)]);
        let result = allocate(&cartoon);
        assert_eq!(result.allocations.len(), 1);
        assert_eq!(result.allocations[0].layer, Layer::Covenant);
        // And it must NOT appear as a vProg guest (the atelier filter is Layer::VProg).
        assert!(
            !result.allocations.iter().any(|a| a.layer == Layer::VProg),
            "a terminal covenant release must not be classified as a vProg"
        );
    }
}
