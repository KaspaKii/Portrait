...
pub struct Hallmark {
    ...
    pub rederive: String,
    /// How the constructor arguments used for the `silverc-accepts` claim were
    /// obtained. [`portrait_emit::GENESIS_PLACEHOLDER_ZERO`] when they are the
    /// emitter's all-zero defaults — which is every covenant that takes params,
    /// since there is no path for supplying real ones. A KovId derived from such
    /// an instance identifies the PLACEHOLDER, not a deployment. `None` when the
    /// covenant takes no params (nothing was defaulted).
    ...
    pub fn to_json(&self) -> String {
        ...
        if let Some(g) = &self.genesis {
            ...
            s.push_str(&format!(
                "  \"genesis_note\": {},\n",
                json_str(
                    "Constructor arguments are the emitter's all-zero defaults. Any KovId or \
                     covenant identity derived from this Hallmark identifies the PLACEHOLDER \
                     instance, not a deployment. Supply real constructor args before deploying."
                )
            ));
        }
        s.push_str("  \"claims\": [\n");
...
pub fn hallmark(path: &Path) -> Result<Hallmark, String> {
    ...
    // H2: record whether the ctor args behind the `silverc-accepts` claim were
    // defaulted, so the artifact self-describes instead of reading as a real
    // deployment identity.
    let genesis = emitted.as_ref().and_then(|pairs| {
        pairs.iter().find_map(|(m, _)| {
            portrait_emit::placeholder_ctor_warning(m)
                .map(|_| portrait_emit::GENESIS_PLACEHOLDER_ZERO.to_string())
        })
    });
