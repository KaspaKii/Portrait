...
> **Time-gate scope note.** Where a pattern below reads "time-delayed", "timelock", or "relative-age delay" (`TimeVault`, `HTLC`, `TimelockedAdmin`, and their compositions), the **currently emitted** covenants gate on a **caller-asserted** coarse time bucket (`now_bucket >= <committed>`), **not** on consensus time — no CLTV / relative-sequence timelock is emitted. The bucket is a spender-supplied argument, so the gate is wallet-assumed, not consensus-enforced. For the exact per-pattern classification of what each emitted `.sil` enforces, see [`library/ENFORCEMENT.md`](../library/ENFORCEMENT.md).
