...
Pool_ctor.json       # constructor args for silverc — ALL-ZERO PLACEHOLDERS (see below)
...
> **`<Name>_ctor.json` is a placeholder, not a deployment.** Portrait fills every
> constructor argument with a type-shaped **zero** — `0` for `int`/`coin`, 32
> zero bytes for a `pubkey`, 64 for a `sig` — because there is no path yet for
> supplying real values. `engrave`/`ship` print a warning saying so, and the
> Hallmark records `"genesis": "placeholder-zero"`. An all-zero `pubkey` is a key
> nobody holds, so a covenant committed to one has a `checkSig` that can never be
> satisfied — deploying it would lock the funds permanently. Put real values in
> `<Name>_ctor.json` (and recompile) before doing anything with the artifact
> beyond checking that it compiles.

The `KovId` is `sha256` of the compiled script bytes. It is the covenant identity
**of whatever constructor arguments were compiled in** — so while
`<Name>_ctor.json` holds the placeholder zeros, the printed KovId identifies the
placeholder instance, *not* your deployment. Note what `ship` does **not** do: it
does not deploy anything. Deploy is opt-in, testnet-only, and handled by a
separate settlement workflow — never mainnet.
