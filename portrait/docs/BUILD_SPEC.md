...
attr       = "#[" , "covenant" , "." , ("singleton"|"") , "(" , "mode" , "=" , ("transition"|"verification") , [ "," , "supply_change" , "=" , Ident ] , ")" , "]" ;
...
invariant  = "invariant" , ( named_inv | temporal_path ) , ";" ;
named_inv     = ( "value_conserved" | "no_undeclared_state" | Ident ) ;
temporal_path = Ident , ":" , Ident , "=>" , "after" , "(" , deadline , ")" ;
deadline      = Ident , [ "+" , Ident ] ;
```

The two invariant forms:

- `named_inv` — the bare predicate-name form (`value_conserved`,
  `no_undeclared_state`, or a `Custom(Ident)` recognizer such as `temporal_guard`
  or `payout_bound`). `payout_bound` is a `Custom` recognizer (no grammar change):
  it requires every recognized settling transition (a `mode = transition` path
  without a `supply_change` capability — a supply change releases no coin) to
  carry a `pays(...)` clause. **`settles` is a
  RECOGNIZER, not a complete settlement detector** — it matches exactly three
  one-shot-flag flip shapes: int-literal (`require f == 0;` + `f: <nonzero>`),
  computed int (`require f == 0;` + `f: f + <nonzero>`), and bool
  (`require f == false;` + `f: true`); a settlement written outside these is NOT
  recognized, and a `payout_bound` that recognizes ZERO settlements is REJECTED
  (fail-loud vacuity). Honest limit: it is EXISTENCE-ONLY — it requires *a*
  `pays(...)` on the settling path (→ `OpTxOutputAmount`/`OpTxOutputSpk`), NOT that
  it binds THIS settlement's own coin/payee (a committed-but-unrelated `pays`
  satisfies it); it is a STRUCTURAL check, NOT a value-conservation / KIP-9 mass
  proof (the L1 surplus caveat applies).
- `temporal_path` (A4-full) — `invariant <name>: <entry> => after(<deadline>);`
  binds the named `entry` transition to carry an `after(<deadline>)` CLTV clause
  with exactly that deadline (the `deadline` grammar is the same `AfterDeadline`
  the `after(...)` statement uses; a `Sum` window matches either operand order).
  `entry` binds by entrypoint NAME across roles — every role declaring an
  entrypoint of that name must carry the matching clause. Honest limit: it is a
  STRUCTURAL check that the named entrypoint CARRIES the matching consensus gate the
  emitter lowers to `OpCheckLockTimeVerify` — strictly stronger than the
  existence-only `temporal_guard`, but NOT an SMT-discharged temporal obligation.

The optional **`supply_change`** covenant-attribute key
(`#[covenant(mode = transition, supply_change = <field>)]`) declares the entry an
EXPLICIT, CHECKED-MODEL supply-change capability. Its value names a committed
authority (a role param or state field). Declaring it (a) WAIVES the entry from
value-conservation checking (`value_conserved` / `conservation_split`) — a supply
change legitimately does not conserve — and (b) is CHECKED unconditionally: the
named authority must be a committed key AND guaranteed to sign on every satisfying
path (a sound, commutative per-key test — the authority's `checkSig(_, <field>)`
must fire under every `&&`/`||` combination, so it is never satisfiable via a
`||` branch or a negated arm), AND the entry must release NO coin (no `pays(...)`
clause and not a terminal spend) — so a supply change provably adjusts committed
supply only, which is what soundly lets `payout_bound` exclude it.
**Honest scope:** this is a checked-model capability, NOT a name convention (the
retired `mint*`/`burn*` heuristic is gone) and NOT an on-chain minted-supply
guarantee — a UTXO covenant cannot inflate real coin; the field is the covenant's
own committed integer, not a mint of L1 KAS.
