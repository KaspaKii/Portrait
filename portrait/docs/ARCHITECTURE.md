...
## 9. Diagnostics — how a rejection is located

Two rejection surfaces, deliberately located by different means.

**Parser (`portrait-syntax`) — source positions.** Every parse rejection funnels
through one `error_at(pos, msg)` renderer, which reports the offending `line:col`,
echoes the offending source line, and points a caret at the column:
...
**Checker (`portrait-sema`) — symbolic locators, not spans.** Sema diagnostics name
the offending declaration rather than a source position: of the 55 `Diagnostic`
construction sites, **51 already name the offending `role.entry`** (for the
conservation and supply-change rules, `role.entry` plus the offending field). The
remaining 4 are app- or lifecycle-level rules with **no single entrypoint to
name** — a lifecycle edge, a flow step, an unreached state, and the app-level
`payout_bound` vacuity check — and each names the role / edge / state / invariant
it is about instead.

**DEFERRED, on purpose: span-carrying sema diagnostics.** Giving sema the same
caret rendering would require threading byte spans through `Role`, `Entry`,
`Param`, `Field` and `Stmt` in `portrait-syntax`, then updating every `Diagnostic`
construction site across the checker and its neighbours. That is a large mechanical
change for **zero correctness value**: it changes no accept/reject decision, and the
symbolic locator already tells an author which declaration was rejected (a sema
rejection is about a *declaration*, not a character). It is deferred until an
editor/LSP integration makes byte spans actually load-bearing, and is recorded here
so the gap is a stated choice rather than an oversight.
